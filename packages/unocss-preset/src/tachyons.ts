...
// Vendored Tachyons vocabulary — we own the table. Scales are data; rules are
// generated from them (DOP). Color utilities resolve to kernel CSS custom
// properties (--ink/--paper/--spot…) so a mode switch re-skins the whole page
// with two attribute writes — no per-utility hex ever ships.
...
const rules: Rule[] = [
  ...
  ...COLOR_TOKENS.map((t): Rule => [t, { color: colorValue(t) }]),
  ...COLOR_TOKENS.map((t): Rule => [`bg-${t}`, { 'background-color': colorValue(t) }]),
  ...COLOR_TOKENS.map((t): Rule => [`b--${t}`, { 'border-color': colorValue(t) }]),
];
